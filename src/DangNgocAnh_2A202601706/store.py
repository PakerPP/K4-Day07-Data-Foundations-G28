...
class EmbeddingStore:
    ...
    def _make_record(self, doc: Document) -> dict[str, Any]:
        """Build the normalized record kept for one document/chunk."""
    ...
    def _search_records(self, query: str, records: list[dict[str, Any]], top_k: int) -> list[dict[str, Any]]:
        """Rank `records` against `query` by dot product and return the top_k as result dicts."""
        if not records:
            return []
    ...
    def search(self, query: str, top_k: int = 5) -> list[dict[str, Any]]:
        ...
        if self._use_chroma and self._collection is not None:
            query_vector = self._embedding_fn(query)
            raw = self._collection.query(query_embeddings=[query_vector], n_results=top_k)
            ids = raw.get("ids", [[]])[0]
    ...
    def delete_document(self, doc_id: str) -> bool:
        ...
        removed_any = len(remaining) < len(self._store)
